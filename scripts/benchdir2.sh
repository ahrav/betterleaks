#!/usr/bin/env bash
# benchdir2.sh - CPU-primary measurement harness for `dir` scans.
#
# Sibling of benchcpu2.sh (identical methodology) for filesystem scans:
# primary metric is CPU-seconds of the whole process tree (getrusage
# children); co-primary is whole-tree peak RSS via a transient systemd
# user-scope cgroup (memory.peak). Wall-clock is recorded but secondary —
# the box is shared. Every row carries the findings digest so keeps can
# demand byte-identical output.
#
# USAGE
#   BIN=./betterleaks EXTRA_ARGS="" benchdir2.sh <label> <path> [runs]
#
# Emits one TSV row:
...
cmd_base = [os.environ["B"], "--no-banner", "-l", "error", "--exit-code", "0",
            "dir", os.environ["C"]] + os.environ["E"].split() + ["-f", "json", "-r", os.environ["R"]]
cmd_base = [c for c in cmd_base if c]

def one():
    unit = f"bld-{os.getpid()}-{random.randint(0,999999)}"
...
one()  # warmup (also populates page cache / dentry cache)
walls, cpus, peak = [], [], 0
for _ in range(runs):
